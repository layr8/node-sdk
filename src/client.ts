...
export class Layr8Client extends EventEmitter {
  ...
  /**
   * Register a catch-all handler for any message type not matched by a
   * specific handler. Adds "*" to the protocol list sent at connect,
   * enabling the cloud-node to route any message type to this agent.
   *
  ...
  private handleInboundMessage(payload: unknown): void {
    ...
    // Route to registered handler (includes catch-all if registered)
